...
const { getAttendees } = require('./../utils/helpers');
const { attendeeToOih } = require('../transformations/attendeeToOih');
...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  */
    ...
    snapshot.lastItem = snapshot.lastItem || (new Date(0)).getTime();

    const objects = await getAttendees(cfg.accessToken, cfg.eventId snapshot);
    ...
    if (objects.length > 0) {
      objects.forEach((elem) => {
        ...
        */
        ...
        const transformedElement = transform(elem, cfg, attendeeToOih);
      ...
      snapshot.lastItem = objects[objects.length - 1].id;
